const fullBlogList = [
  {
    id: "2afcb6d5-32e5-4b33-becb-83dee222b3c2",
    title: "Introduction to Software Testing",
    description:
      "Software testing is the process of evaluating a software system or application to ensure that it meets its requirements and works as intended. It involves executing software under controlled conditions and using a variety of techniques and tools to identify defects, errors, and other issues that could impact the quality, functionality, or performance of the software.",
    createdAt: "2023-04-03T02:40:44.000Z",
    updatedAt: "2023-04-03T02:40:44.000Z",
    authorId: "11af8088-2fd6-449b-9b57-7cc36e757ab1",
    user: {
      user: "test",
    },
  },
  {
    id: "d8d94b18-599b-4f1e-a708-a6ee7db7818f",
    title: "Introduction to Information Security",
    description:
      "Information security, also known as cybersecurity or IT security, is the practice of protecting computer systems, networks, and sensitive information from unauthorized access, use, disclosure, disruption, modification, or destruction. It involves using various measures and technologies to safeguard digital assets and prevent them from being compromised, stolen, or lost.",
    createdAt: "2023-04-03T02:30:03.000Z",
    updatedAt: "2023-04-03T02:30:03.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
    user: {
      user: "tahmeed",
    },
  },
  {
    id: "1ee6b779-e057-49d4-941f-4ea8141c331e",
    title: "Introduction to Software Testing",
    description:
      "Software testing is the process of evaluating a software system or application to ensure that it meets its requirements and works as intended. It involves executing software under controlled conditions and using a variety of techniques and tools to identify defects, errors, and other issues that could impact the quality, functionality, or performance of the software.",
    createdAt: "2023-04-03T02:30:01.000Z",
    updatedAt: "2023-04-03T02:30:01.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
    user: {
      user: "tahmeed",
    },
  },
  {
    id: "ee5a5f83-c1d2-47f4-8540-3a4d2043eaa8",
    title: "Introduction to Software Design",
    description:
      "Software design is the process of creating a plan or blueprint for a software system or application that meets the requirements of users, stakeholders, and customers. It involves translating the software requirements into a detailed design that specifies how the software will be structured, organized, and implemented.",
    createdAt: "2023-04-03T02:29:58.000Z",
    updatedAt: "2023-04-03T02:29:58.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
    user: {
      user: "tahmeed",
    },
  },
  {
    id: "76193069-6f69-430a-9c0d-6d62a917a34f",
    title: "Introduction to Software Testing",
    description:
      "Software testing is the process of evaluating a software system or application to ensure that it meets its requirements and works as intended. It involves executing software under controlled conditions and using a variety of techniques and tools to identify defects, errors, and other issues that could impact the quality, functionality, or performance of the software.",
    createdAt: "2023-04-03T02:29:55.000Z",
    updatedAt: "2023-04-03T02:29:55.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
    user: {
      user: "tahmeed",
    },
  },
  {
    id: "d3673413-685c-42d5-b93c-25494cc1a825",
    title: "Introduction to Software Security",
    description:
      "Software security is the practice of designing, implementing, and testing software systems and applications in a way that ensures they are protected from malicious attacks, vulnerabilities, and other security threats. It involves using a combination of software engineering, computer science, and security principles to build secure software that is resilient to attacks and breaches.",
    createdAt: "2023-03-31T06:32:21.000Z",
    updatedAt: "2023-03-31T06:32:21.000Z",
    authorId: "5cabfa95-df2f-4d06-bddf-50c21847c13c",
    user: {
      user: "arif",
    },
  },
  {
    id: "74d21f82-4ddc-4e96-a5c2-ee95d3ca62f9",
    title: "Introduction to Software Testing",
    description:
      "Software testing is the process of evaluating a software system or application to ensure that it meets its requirements and works as intended. It involves executing software under controlled conditions and using a variety of techniques and tools to identify defects, errors, and other issues that could impact the quality, functionality, or performance of the software.",
    createdAt: "2023-03-31T05:05:07.000Z",
    updatedAt: "2023-03-31T05:05:07.000Z",
    authorId: "2ed99fff-e742-4a26-b840-c2a7248f351a",
    user: {
      user: "rakib",
    },
  },
  {
    id: "7f24c257-2619-4e1e-9527-4997fbf66575",
    title: "Introduction to Software Metrics",
    description:
      "Software metrics are quantitative measures of software products, processes, and projects that help to assess their quality, performance, and efficiency. They provide objective data that can be used to evaluate the effectiveness of software development and maintenance activities, identify areas for improvement, and track progress towards project goals.",
    createdAt: "2023-03-31T05:04:45.000Z",
    updatedAt: "2023-03-31T05:04:45.000Z",
    authorId: "2ed99fff-e742-4a26-b840-c2a7248f351a",
    user: {
      user: "rakib",
    },
  },
  {
    id: "2387a49e-403f-4973-9e24-8a231037efa0",
    title: "Introduction to Information Security",
    description:
      "Information security, also known as cybersecurity or IT security, is the practice of protecting computer systems, networks, and sensitive information from unauthorized access, use, disclosure, disruption, modification, or destruction. It involves using various measures and technologies to safeguard digital assets and prevent them from being compromised, stolen, or lost.",
    createdAt: "2023-03-31T05:04:30.000Z",
    updatedAt: "2023-03-31T05:04:30.000Z",
    authorId: "2ed99fff-e742-4a26-b840-c2a7248f351a",
    user: {
      user: "rakib",
    },
  },
  {
    id: "66480a5f-955f-4240-9ca3-9cdd56af68c4",
    title: "Introduction to Computer Networking",
    description:
      "Computer networking refers to the practice of connecting multiple computers, devices, and other equipment together to share resources, exchange information, and communicate with one another. It involves using hardware and software technologies to create and manage computer networks, which may be local area networks (LANs) within a single building or campus, wide area networks (WANs) connecting geographically dispersed locations, or other types of networks such as metropolitan area networks (MANs) or storage area networks (SANs).",
    createdAt: "2023-03-31T05:04:16.000Z",
    updatedAt: "2023-03-31T05:04:16.000Z",
    authorId: "2ed99fff-e742-4a26-b840-c2a7248f351a",
    user: {
      user: "rakib",
    },
  },
];

const authorFullBlogList = [
  {
    id: "1ee6b779-e057-49d4-941f-4ea8141c331e",
    title: "Introduction to Software Testing",
    description:
      "Software testing is the process of evaluating a software system or application to ensure that it meets its requirements and works as intended. It involves executing software under controlled conditions and using a variety of techniques and tools to identify defects, errors, and other issues that could impact the quality, functionality, or performance of the software.",
    createdAt: "2023-04-03T02:30:01.000Z",
    updatedAt: "2023-04-03T02:30:01.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
  },
  {
    id: "76193069-6f69-430a-9c0d-6d62a917a34f",
    title: "Introduction to Software Testing",
    description:
      "Software testing is the process of evaluating a software system or application to ensure that it meets its requirements and works as intended. It involves executing software under controlled conditions and using a variety of techniques and tools to identify defects, errors, and other issues that could impact the quality, functionality, or performance of the software.",
    createdAt: "2023-04-03T02:29:55.000Z",
    updatedAt: "2023-04-03T02:29:55.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
  },
  {
    id: "d8d94b18-599b-4f1e-a708-a6ee7db7818f",
    title: "Introduction to Information Security",
    description:
      "Information security, also known as cybersecurity or IT security, is the practice of protecting computer systems, networks, and sensitive information from unauthorized access, use, disclosure, disruption, modification, or destruction. It involves using various measures and technologies to safeguard digital assets and prevent them from being compromised, stolen, or lost.",
    createdAt: "2023-04-03T02:30:03.000Z",
    updatedAt: "2023-04-03T02:30:03.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
  },
  {
    id: "ee5a5f83-c1d2-47f4-8540-3a4d2043eaa8",
    title: "Introduction to Software Design",
    description:
      "Software design is the process of creating a plan or blueprint for a software system or application that meets the requirements of users, stakeholders, and customers. It involves translating the software requirements into a detailed design that specifies how the software will be structured, organized, and implemented.",
    createdAt: "2023-04-03T02:29:58.000Z",
    updatedAt: "2023-04-03T02:29:58.000Z",
    authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
  },
];
const singleBlog = {
  id: "ee5a5f83-c1d2-47f4-8540-3a4d2043eaa8",
  title: "Introduction to Software Design",
  description:
    "Software design is the process of creating a plan or blueprint for a software system or application that meets the requirements of users, stakeholders, and customers. It involves translating the software requirements into a detailed design that specifies how the software will be structured, organized, and implemented.",
  createdAt: "2023-04-03T02:29:58.000Z",
  updatedAt: "2023-04-03T02:29:58.000Z",
  authorId: "acd6f5ae-5c57-47aa-996d-5a877f9bf0fb",
};

module.exports = { fullBlogList, authorFullBlogList, singleBlog };
